...
| **Thẩm định học liệu** bởi giáo viên VSTEP                                         | Mọi bài vẫn hiện "Chưa qua thẩm định". Nay chỉ còn một thao tác: `/review-pack/bank` in ra sáu tập hồ sơ (ngữ liệu, câu hỏi, đáp án có đánh dấu, ô trống để ghi) để gửi đi. Kết quả về thì sửa tay `provenance.ts` với tên thật và ngày thật                                                                                              |
| **Vercel chỉ promote sau khi CI xanh**, bảo vệ nhánh `main`, kênh nhận lỗi, uptime | Cấu hình nằm ngoài repository. Phần trong repo đã xong: `scripts/vercel-ignore-build.mjs` chặn build khi CI của đúng SHA chưa xanh — còn hai ô cần anh điền trên Vercel (Ignored Build Step và `MAY_CI_TOKEN`), xem RELEASE-RUNBOOK.md. Kênh nhận lỗi: nút **Gửi báo lỗi** trong Cài đặt đã đưa bản mô tả vào khay chia sẻ của điện thoại |
| **UAT trên đúng điện thoại Gùa**                                                   | Cần thiết bị thật và thời gian của hai người. Danh sách đã soạn sẵn: [UAT-DIEN-THOAI.md](UAT-DIEN-THOAI.md), 20 mục, 25–35 phút, chỉ gồm thứ máy ảo không kiểm được. _(Diễn tập khôi phục đã được tự động hoá thành một ca E2E: xuất bản sao, xoá sạch máy, nhập lại, kiểm lịch sử.)_                                                     |
