# Mốc tiếp tục công việc — 11/09/2026
...
- Đã có bản HTTPS pilot tại `https://vstep-turtle.vercel.app`: 9 màn chính và toàn bộ 14 bài luyện tải đúng, route giả trả 404, security headers đúng và giao diện production có learning loop mới. Ngày 11/09/2026, toàn bộ Reading/Listening đã được crawl ở desktop/mobile; luồng Listening phát/dừng, chấm điểm, phân tích và mở transcript sau khi nộp không có lỗi runtime. Supabase production đã được cấu hình; chưa đăng nhập/sync bằng tài khoản thật trên host, chưa thử micro hoặc thiết bị iOS/Android thật.
