...
| F14 | Đã đo, chưa xử lý | Sổ lỗi sinh thẻ trùng cho 20 câu Đọc mà đề đầy đủ dùng lại: khóa ôn là `lesson@version:question` nên `reading-cafe@v1:rc1` và `full-reading-cafe@v1:full-rc1` không gộp. Sau một buổi thi đủ cấu trúc có thể có tới 20 thẻ trùng từng chữ, và `wrongCount` bị chia đôi nên một hiểu nhầm dai dẳng bị xếp hạng thấp hơn thực tế. Cách sửa gốc là thay 20 câu dùng lại bằng câu mới (P1-04), không phải đổi cách đánh khóa. | P1-04 |
| F15 | Đã đo, chưa xử lý | `skillStats` lấy 5 lượt gần nhất, trong khi một buổi thi đủ cấu trúc ghi 14 lượt Nghe riêng lẻ. Một buổi thi quét sạch toàn bộ tín hiệu Nghe trước đó và thay bằng 5 ngữ liệu cuối của đề; con số này lại điều khiển kế hoạch ngày. Nên tính theo số câu gần nhất hoặc ghi một lượt cho mỗi phần thi thay vì mỗi ngữ liệu. | P1-05 |
| F16 | Đã đo, chưa xử lý | Trang luyện và phòng thi gọi `updateStudy` mỗi giây, mà mỗi lần gọi đều đọc lại, parse và kiểm tra toàn bộ state bằng Zod. Đo trên máy chủ: 26 lượt học ≈ 111 KB mất 3,3 ms; 104 lượt ≈ 444 KB mất 7,5 ms — điện thoại chậm hơn nhiều lần. Nên nhấc điều kiện hết giờ ra ngoài reducer và gom nháp 10–15 giây một lần. | P2 hiệu năng |
| F17 | Xác nhận từ code, chưa xử lý | Không có micro thì phần Nói không thể hoàn thành ở bất kỳ đâu: trang luyện chặn nộp, phòng thi chỉ tạo lượt từ callback ghi âm. Ba bài Nói vì thế giữ mãi điểm thưởng “chưa từng luyện” trong `todayPlan` và chiếm một suất kế hoạch ngày vô thời hạn, lấn sang ưu tiên Reading/Listening. | P1 Speaking |
| F18 | Xác nhận từ code, chưa xử lý | Bản ghi chỉ nằm trong IndexedDB thường, không gọi `navigator.storage.persist()` nên có thể bị trình duyệt thu hồi; sao lưu JSON và cloud đều không chứa audio. Hướng dẫn trong Cài đặt lại chỉ người học sang trang bài Nói, nơi chỉ tải được bản mới nhất của mỗi bài — bản của từng lượt chỉ có ở Lịch sử. Chưa có nút xóa nên mỗi lần nộp còn để lại thêm một bản sao vĩnh viễn. | P2 |
| F19 | Xác nhận từ code, chưa xử lý | Chỉ số ngữ liệu đang làm trong phòng thi chỉ nằm ở state component, không lưu vào `examSchema`. Tải lại trang giữa phần Đọc 60 phút sẽ quay về bài 1/4 trong khi đồng hồ vẫn chạy. | P2 |
...
| F13 | Đã đo, chưa xử lý | Phân bố dạng câu lệch: phần Đọc của đề đủ cấu trúc có 24/40 câu “Thông tin chi tiết”, chỉ 2 câu từ vựng trong ngữ cảnh và 1 câu từ tham chiếu; phần Nghe có 29/35 câu chi tiết. Cần ma trận dạng câu trước khi mở rộng ngân hàng. | P1-01/P1-04 |
